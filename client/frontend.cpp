...
bool Application::Frontend::is_client = false;
...
std::queue<std::pair<int, int>> Application::Frontend::GetCommands() {
  return Commands;
}

void Application::Frontend::SetIsClient(bool is_client) {
  Application::Frontend::is_client = is_client;
}
...
void Application::Frontend::processInputInQueue(GLFWwindow* window) {
  if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS)
    glfwSetWindowShouldClose(window, true);
